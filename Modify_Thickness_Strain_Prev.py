...
Thickness_Profile_Prev=np.loadtxt('Thickness_Profile.txt')
Strain_Profile=np.loadtxt('Strain_Profile.txt')
Strain_Profile_Prev=np.loadtxt('Strain_Profile_Prev.txt')
...
Strain_Profile_Prev=Strain_Profile_Prev[Strain_Profile_Prev[:, 0].argsort()]
Strain_Profile_n[:,0]=coords[:,0]
Strain_Profile_n=Strain_Profile_n[Strain_Profile_n[:, 0].argsort()]
Strain_Profile_n[:,1]=np.interp(Strain_Profile_n[:,0],Strain_Profile_Prev[:,0],Strain_Profile_Prev[:,1])
...
def Get_fun(Strain_Profile_loc,Strain_Profile_val,Strain_Profile_Prev_loc,Strain_Profile_Prev_val,coordsx,coordsy,maxcoordsx,maxcoordsy):

	Diff= (Strain_Profile_val-Strain_Profile_Prev_val)*0.1

	Val=(Diff)*0.5*Edge_Height
	return Val	
...
for i in range(len(coords[:,0])):
	if coords[i,1]< -Edge_Height/2+2:
		#print(1)
		coords[i,1]=coords[i,1]-Get_fun(Strain_Profile[i,0],Strain_Profile[i,1],Strain_Profile_Prev[i,0],Strain_Profile_Prev[i,1],coords[i,0],coords[i,1],np.amax(coords[:,0]),np.amax(coords[:,1]))-(Thickness_Profile_Prev[i,1]-1)#-np.abs(coords[i,0])/np.amax(coords[:,0])*300#*0

	elif coords[i,1]> -Edge_Height/2+2:
		Old_Bot= -Edge_Height/2
		Old_Top= Edge_Height/2
		Loc_norm= (coords[i,1]-Old_Bot)/(Old_Top-Old_Bot)
		New_Bot= Old_Bot-Get_fun(Strain_Profile[i,0],Strain_Profile[i,1],Strain_Profile_Prev[i,0],Strain_Profile_Prev[i,1],coords[i,0],Old_Bot,np.amax(coords[:,0]),np.amax(coords[:,1]))-(Thickness_Profile_Prev[i,1]-1)
...
for i in range(len(Thickness)):
	Thickness[i]=Edge_Height+Get_fun(Strain_Profile[i,0],Strain_Profile[i,1],Strain_Profile_Prev[i,0],Strain_Profile_Prev[i,1],coords[i,0],coords[i,1],np.amax(coords[:,0]),np.amax(coords[:,1]))#+np.abs(coords[i,0])/np.amax(coords[:,0])*300#*0
